...
Every module must be registered in the corresponding registry and has its own folder inside the \texttt{src/overlays} or \texttt{src/screens} directory.
...
But be careful to not make the underlying screen unclickable.
...
In this section you will learn how to create the settings component for screen and overlay modules.
...
\begin{itemize}
  ...
  \item \texttt{onChange}: callback that is called when the value changed. 
    ...
    The first is the name of teh property (passed via the \texttt{name} property) and the second is the new value.
